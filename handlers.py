...
async def command_low_handler(message: Message):
    ...
    if isinstance(data, list):
        if len(data) >= 10:
            movie_list = "\n".join([f"{len(data) - 10 + i + 1}. {movie['title']} ({movie['year']})" for i, movie in
                                    enumerate(data[-10:])])
            await message.answer(text=f"10 фильмов с наименьшим рейтингом:\n{movie_list}")
        else:
...
async def command_high_handler(message: Message):
    ...
    if isinstance(data, list):
        if len(data) >= 10:
            movie_list = "\n".join([f"{i + 1}. {movie['title']} ({movie['year']})" for i, movie in
                                    enumerate(data[:10])])
            await message.answer(text=f"10 фильмов с наибольшим рейтингом:\n{movie_list}")
        else:
            await message.answer(text="В списке недостаточно фильмов.")
    else:
        await message.answer(text=f"Ошибка: {data.get('error', 'Неизвестная ошибка')}")
...
async def command_custom_handler(message: Message):
    ...
    if len(args) == 3:
        try:
            ...
            data = await fetch_movies()
            if start > end:
                await message.answer(text="Начальное число должно быть меньше конечного.")
                await message.answer(text="Пожалуйста, введите два числа для диапазона, например: /custom 1 10.")
                return
            elif start < 0 or end < 0:
                await message.answer(text="Начальное и конечное числа должны быть положительными.")
                return
            elif isinstance(data, list):
                if start < 0 or end > len(data):
                    await message.answer(text="Диапазон выходит за пределы доступных фильмов.")
                    return

                movie_list = "\n".join(
                    [f"{i + 1}. {movie['title']} ({movie['year']})" for i, movie in enumerate(data[start:end])])
                await message.answer(text=f"Фильмы в диапазоне с {start + 1} по {end}:\n{movie_list}")
        ...
        await message.answer(text="Команда должна содержать два числа для диапазона, например: /custom 1 10.")
...
async def command_history_handler(message: Message):
    ...
    if history:
        history_text = "\n".join([f"{i + 1}. {cmd} — {timestamp}" for i, (cmd, timestamp) in enumerate(history)])
        await message.answer(text=f"История ваших последних 10 запросов:\n{history_text}")
    else:
